...
# Подсети /18
subnets = [
    "85.195.64."  # Подсеть /18
    # Добавьте другие подсети /18 по мере необходимости
]
...
def create_ip_list(subnets):
    ...
    for subnet in subnets:
        base = subnet.split('.')
        for i in range(64):  # Для подсети /18 шаг 64
            for j in range(256):
...
def process_pings():
    ...
    if high_ping_ips:
        ...
        for chunk in chunks:
            ips = ','.join(chunk)
            rule_name = f"HighPingServersApex/18_{rule_number}"
            try:
